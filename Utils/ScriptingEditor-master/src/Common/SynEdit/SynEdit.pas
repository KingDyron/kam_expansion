...
{$ENDIF}
    ...
    procedure SetAdditionalIdentChars(Value: TSysCharSet);
    procedure SetAdditionalWordBreakChars(Value: TSysCharSet);
...
begin
  if not HandleAllocated or (PaintLock <> 0) then
    ...
    begin
      ...
      if (fScrollBars in [{$IFDEF SYN_COMPILER_17_UP}TScrollStyle.{$ENDIF}ssBoth, {$IFDEF SYN_COMPILER_17_UP}TScrollStyle.{$ENDIF}TScrollStyle.ssHorizontal]) and not WordWrap then
      begin
        if eoScrollPastEol in Options then
...
procedure TCustomSynEdit.SetAdditionalIdentChars(Value: SysUtils.TSysCharSet);
begin
  FAdditionalIdentChars := Value;
end;

procedure TCustomSynEdit.SetAdditionalWordBreakChars(Value: SysUtils.TSysCharSet);
begin
  ...
  begin
    ...
    begin
      Result := not CollectionsEqual(Keystrokes,
        TCustomSynEdit(Filer.Ancestor).Keystrokes, nil, nil);
    end
    else begin
      iDefKeys := TSynEditKeyStrokes.Create(nil);
      try
        iDefKeys.ResetDefaults;
        Result := not CollectionsEqual(Keystrokes, iDefKeys, nil, nil);
      finally
        iDefKeys.Free;
      end;
    end;
