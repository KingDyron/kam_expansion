...
type
  TProposalType = (ptActions, ptStates, ptEvents, ptUtils, ptLocal);
  TProposalTypeSet = set of TProposalType;
  ...
  private
    ...
    fTypeButtons : array[TProposalType] of TButton;
    ...
    procedure TypeButtonClicked(Sender : TObject);
  protected
    ...
    procedure FormChanged(Sender : TObject; aIndex : Integer);
  ...
  private
    ...
    function IsWordBreakChar(AChar: WideChar): Boolean;
    procedure FormTypeChange(Sender : TObject);
  protected
  ...
  published
    ...
    property ProposalTypes : TProposalTypeSet read fPropTypes;
...
{$ENDIF}
  ...
  for PT := Low(TProposalType) to High(TProposalType) do
  begin
    fTypeButtons[PT] := TButton.Create(self);
    fTypeButtons[PT].Parent := self;
    fTypeButtons[PT].Enabled := true;
    fTypeButtons[PT].Left := 100 + byte(PT) * 100;
    fTypeButtons[PT].Top := 0;
    fTypeButtons[PT].Width := 90;
    fTypeButtons[PT].Height := 15;
    fTypeButtons[PT].OnClick := TypeButtonClicked;
    fTypeButtons[PT].Font.Style := [fsBold, fsUnderline];
    fTypeButtons[PT].Enabled := false;
    case PT of
      ptActions: fTypeButtons[PT].Caption := 'Actions';
      ptStates: fTypeButtons[PT].Caption := 'States';
      ptEvents: fTypeButtons[PT].Caption := 'Events';
      ptUtils: fTypeButtons[PT].Caption := 'Utils';
      ptLocal: fTypeButtons[PT].Caption := 'Local';
    end;
  end;
  Visible := False;
...
procedure TSynBaseCompletionProposalForm.FormChanged(Sender: TObject; aIndex: Integer);
begin
...
procedure TSynBaseCompletionProposalForm.TypeButtonClicked(Sender: TObject);
var PT : TProposalType;
begin

  for PT := Low(TProposalType) to High(TProposalType) do
    if Sender = fTypeButtons[PT] then
    begin
      if fsBold in fTypeButtons[PT].Font.Style then
        fTypeButtons[PT].Font.Style := []
      else
        fTypeButtons[PT].Font.Style := [fsBold, fsUnderline];

    end;
  if Visible and Assigned(FOnTypeChange) and (DisplayType = ctCode) then
    FOnTypeChange(nil);
end;
...
begin
  ...
  fForm.OnTypeChange := FormTypeChange;
...
procedure TSynBaseCompletionProposal.FormTypeChange(Sender : TObject);
var PT: TProposalType;
begin
  fPropTypes := [];

  for PT := Low(TProposalType) to High(TProposalType) do
    If fsBold in fForm.fTypeButtons[PT].Font.Style then
      fPropTypes := fPropTypes + [PT];
