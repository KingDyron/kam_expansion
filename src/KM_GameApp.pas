...
begin
  ...
  begin
    ...
    try
      S.Write(fGithubCRC, SizeOf(fGithubCRC));
      S.SaveToFile(ExeDir + 'data\save\CRC.dat');
    finally
      S.Free;
    end;
  end;
  if fGithubCRC = 0 then
  begin
    //try to read the latest loaded CRC from data\save\CRC.dat
    if FileExists(ExeDir + 'data\save\CRC.dat') then
    begin
      S := TMemoryStream.Create;
      try
        S.LoadFromFile(ExeDir + 'data\save\CRC.dat');
        S.Read(fGithubCRC, SizeOf(fGithubCRC));
