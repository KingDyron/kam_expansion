...
begin
  ...
  begin
    ...
    begin
      ...
      for K := 0 to high(house.HSpec.Workers) do //set how many workers house already has
        tmpUnitCnt[house.HSpec.Workers[K].UnitType] := house.WorkersCount(house.HSpec.Workers[K].UnitType);

      J := house.MaxWorkers - house.WorkersCount; //remember how many free space we have, f.e 10-3 = 7 free space
      while J > 0 do
...
var
  ...
  begin
    ...
    for I := Low(SIEGE_GAME_ORDER) to High(SIEGE_GAME_ORDER) do
      if gHands[fOwner].Locks.GetUnitBlocked(SIEGE_GAME_ORDER[I]) = ulUnlocked then
        Exit(true);

  end;

  function UnitUnlocked(aType : TKMUnitType) : Boolean;
  begin
    Result :=  gHands[fOwner].Locks.GetUnitBlocked(aType) = ulUnlocked;
  end;
      ...
      htStoneWorkshop:   for K := 1 to WARES_IN_OUT_COUNT do
                          If H.WareOrder[K] = 0 then
                            case H.WareOutput[K] of
                              ...
                              wtLog : If MachinesUnlocked and (ArmyDemand[gtMachines] + ArmyDemand[gtMachinesMelee] > 0)then H.WareOrder[K] := 6;
                              wtWheel : If MachinesUnlocked and (ArmyDemand[gtMachines] + ArmyDemand[gtMachinesMelee] > 0) then H.WareOrder[K] := 3;
...
procedure TKMayor.SetArmyDemand(aDemand : TAIArmyDemand);
  ...
  begin
    if aIron then
      case aGT of
        gtMelee:     Result := gHands[fOwner].Locks.GetUnitBlocked(utSwordFighter) = ulUnlocked;
        gtAntiHorse: Result := gHands[fOwner].Locks.GetUnitBlocked(utPikeman) = ulUnlocked;
        gtRanged:    Result := gHands[fOwner].Locks.GetUnitBlocked(utCrossbowman) = ulUnlocked;
        gtMounted:   Result := gHands[fOwner].Locks.GetUnitBlocked(utKnight) = ulUnlocked;
        gtWreckers:   Result := gHands[fOwner].Locks.GetUnitBlocked(utFlailFighter) = ulUnlocked;
        else         Result := True;
      end
    else
      case aGT of
        gtMelee:     Result := (gHands[fOwner].Locks.GetUnitBlocked(utMilitia) = ulUnlocked) and
                                (gHands[fOwner].Locks.GetUnitBlocked(utAxeFighter) = ulUnlocked);
        gtAntiHorse: Result := gHands[fOwner].Locks.GetUnitBlocked(utLanceCarrier) = ulUnlocked;
        gtRanged:    Result := gHands[fOwner].Locks.GetUnitBlocked(utBowman) = ulUnlocked;
        gtMounted:   Result := gHands[fOwner].Locks.GetUnitBlocked(utScout) = ulUnlocked;
        gtWreckers:   Result := gHands[fOwner].Locks.GetUnitBlocked(utMaceFighter) = ulUnlocked;
        else         Result := True;
      end;
  end;
  ...
  begin
    if gHands[fOwner].Locks.GetUnitBlocked(aUT) <> ulUnlocked then
...
begin
  ...
  case aWare of
    ...
    wtSkin: Result := HasHouses([htTannery]) and not HasHouses([htSwine]);
    ...
    wtBitinE: Result := ((ArmyDemand[gtMachines] > 0) or (ArmyDemand[gtMachinesMelee] > 0))
                        and gHands[fOwner].Locks.UnitsUnlocked([utSwordFighter, utCrossbowman, utPikeman, utKnight, utCatapult, utBallista, utWarrior, utRam])
                        and HasHouses([htSiegeWorkshop, htBarracks]);
    wtLog,
    wtSteelE,
    ...
    wtBolt: Result :=  HasUnits([utBallista]);

    wtQuiver: Result := HasUnits([utBowman, utCrossBowMan])
                        or HasHouses([htWallTower]);

    wtWater: Result := HasHouses([htHovel, htBakery, htStables, htSwine, htAppletree]);
