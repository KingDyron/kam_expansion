...
begin
  ...
  Scroll_Count := TKMTrackBar.Create(Panel_WaresOnGround, 9, 27, Panel_WaresOnGround.Width - 18, 0, 15);
  Scroll_Count.Caption := gResTexts[1040];
  Scroll_Count.OnChange := Wares_Change;
  Scroll_Count.ThumbWidth := 100;

  with TKMLabel.Create(Panel_WaresOnGround, 0, 67, Panel_WaresOnGround.Width, 0, gResTexts[2074], fntGrey, taCenter) do
    Anchors := [anLeft, anTop, anRight];
  ...
  for WT := WARE_MIN to WARE_MAX do
    ...
    begin
      ...
      Button_Wares[I] := TKMButtonFlat.Create(Panel_WaresOnGround, 9 + I mod 5 * 34, 85 + I div 5 * 34, 30, 30, gRes.Wares[WT].GUIIcon);
...
begin
  ...
  PopUp_Patterns := TKMPopUpMenu.Create(aParent.MasterPanel, aParent.MasterPanel.Width div 2);
  ...
  TKMImage.Create(PopUp_Patterns, 0, 0, PopUp_Patterns.Width, PopUp_Patterns.Height, 15, rxGuiMain).ImageStretch;
    ...
    Button_RefreshPatterns:= TKMButton.Create(PopUp_Patterns, ColumnBox_PatternsList.Right - 150, ColumnBox_PatternsList.Bottom + 5, 150, 25, gResTexts[1830], bsGame);
    Button_RefreshPatterns.OnClick := ObjectsPatternClick;

    Button_ReName:= TKMButton.Create(PopUp_Patterns, ColumnBox_PatternsList.Left, ColumnBox_PatternsList.Bottom + 5, 150, 25, gResTexts[1831], bsGame);
