...
type
  TKMMenuCampaignLobby = class(TKMMenuPageCommon)
  private
...
{ TKMGUIMainCampaign }
constructor TKMMenuCampaignLobby.Create(aParent: TKMPanel; aCampaigns: TKMCampaignsCollection; aOnPageChange: TKMMenuChangeEventText);
var
  I: Integer;
begin
  ...
  Button_CampaignStart := TKMButton.Create(Panel_Campaign, aParent.Width-220-20, aParent.Height-50, 220, 30, 'Select Map', bsMenu);
...
procedure TKMMenuCampaignLobby.RefreshCampaign;
const
...
procedure TKMMenuCampaignLobby.UpdateDifficultyLevel;
var
  I: Integer;
...
procedure TKMMenuCampaignLobby.SelectMap(aMapIndex: Byte);
var
  I, panHeight: Integer;
  color: Cardinal;
begin
...
procedure TKMMenuCampaignLobby.Campaign_SelectMap(Sender: TObject);
var I : Integer;
begin
...
procedure TKMMenuCampaignLobby.PlayBriefingAudioTrack;
begin
...
procedure TKMMenuCampaignLobby.StartClick(Sender: TObject);
begin
  gMusic.StopPlayingOtherFile;
  MapSelected := fMapIndex;
  gMusic.StopPlayingOtherFile; //Cancel briefing if it was playing
  Hide;
  if Assigned(OnMapSelected) then
    OnMapSelected(Self);
end;


procedure TKMMenuCampaignLobby.Difficulty_Change(Sender: TObject);
begin
...
procedure TKMMenuCampaignLobby.AnimNodes(aTickCount: Cardinal);
begin
...
function TKMMenuCampaignLobby.Visible: Boolean;
begin
  Result := Panel_Campaign.Visible;
end;


procedure TKMMenuCampaignLobby.UpdateState(aTickCount: Cardinal);
begin
...
procedure TKMMenuCampaignLobby.Resize(X, Y: Word);
var
  I: Integer;
begin
  if (fCampaign = nil) or not Visible then Exit;
...
procedure TKMMenuCampaignLobby.MouseMove(Shift: TShiftState; X,Y: Integer);
begin
  //
end;


procedure TKMMenuCampaignLobby.Show(aCampaign: TKMCampaignId);
begin
...
procedure TKMMenuCampaignLobby.Hide;
begin
  Panel_Campaign.Hide;
end;

procedure TKMMenuCampaignLobby.BackClick(Sender: TObject);
begin
  gMusic.StopPlayingOtherFile; //Cancel briefing if it was playing
  Hide;
end;
...
procedure TKMMenuCampaignLobby.Scroll_Toggle(Sender: TObject);
begin
