...
type
  ...
  protected
    ...
    Button_ToggleVegeType: TKMButton;
...
begin
  ...
  Panel_House.DoScrollUpdate := false;
    ...
    Image_WorkProgress := TKMImageAnimation.Create(Panel_House, Panel_House.Width - 65, 70, 20, 20, 0, rxGui);
    ...
    Button_ToggleGrainType := TKMButton.Create(Panel_House, TB_WIDTH - 110, 42 + 30, 30, 25, GRAIN_GUI_PIC[gftWheat], rxGui, bsGame );
    ...
    Button_ToggleGrassType := TKMButton.Create(Panel_House, TB_WIDTH - 80, 42 + 30, 30, 25, GRAIN_GUI_PIC[gftWheat], rxGui, bsGame );
    ...
    Button_ToggleVegeType := TKMButton.Create(Panel_House, TB_WIDTH - 50, 42 + 30, 30, 25, GRAIN_GUI_PIC[gftWheat], rxGui, bsGame );
...
begin
  ...
  begin
    ...
    Coal_Image[I] := TKMCostsRow.Create(Panel_HouseWoodBurner, 0, Wood_Bar[I].Top, TB_WIDTH - 5, 20);
...
begin
  ...
  demandTop := 10;
  if Button_ForceWork.Visible or Button_ToggleGrainType.Visible then
    Inc(demandTop, 30);
  ...
  begin
    ...
    demandTop :=30 +  30 * ( high(gRes.Houses[aHouse.HouseType].WareInputSlots) div 5);
  end;
  ...
  case aHouse.HouseType of
    ...
    htMerchant:       begin
                        ...
                        for I := 0 to high(Button_PlayerSelect) do
                          ...
                          begin
                            Button_PlayerSelect[I].Top := base + 30 * (Line div 6);
    ...
    begin
      ...
      htSwine :     begin
                      ...
                      Progress_Beasts.TexID := [[0, 490, 569, 403]];
                    end;
        htStables : begin
                      Progress_Beasts.Progress := TKMHouseSwineStable(aHouse).GetBeastsProgresses;
                      Progress_Beasts.Colors := TKMHouseSwineStable(aHouse).GetBeastsProgressColors;
                      Progress_Beasts.TexID := [[0, 1538, 1538]];
                    end;
        htHovel :   begin
                      Progress_Beasts.Progress := TKMHouseHovel(aHouse).GetBeastsProgresses;
                      Progress_Beasts.Colors := [icLightGreen];
                      Progress_Beasts.TexID := [[0, 2378, 2378, 2378]];
                    end;
        htFarm :    begin
                      Progress_Beasts.RX := rxGui;
    ...
    begin
      ...
      for I := 1 to WARES_IN_OUT_COUNT do
        ...
        begin
          ...
          WaresRow_Common[RowRes].Top       := Base + Line * LINE_HEIGHT;
          WaresRow_Common[RowRes].WareCntAsNumber := false;
          WaresRow_Common[RowRes].Hitable := true;
          WaresRow_Common[RowRes].HideHighlight := false;
          WaresRow_Common[RowRes].Clickable := true;
          WaresRow_Common[RowRes].Show;
          ...
          case aHouse.HouseType of
            htHovel : If W = wtVegetables then WaresProdCt_Common[RowRes].Caption := 'x' + IntToStr(TKMHouseHovel(aHouse).ChickenCount) else
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        WareOrderRow_Order[I].AsCheckBox := fHouse.HouseType = htProductionThatch;
        if W = wtSawDust then
    ...
    begin
      ...
      begin
        ...
        CostsRow_Costs[I].Top := Base + lastLine * LINE_HEIGHT + (I - 1) * 42; //Pack them closer so they fit on 1024x576
        Inc(Line);
      end;
    end;
  end;
...
begin
  ...
  for K := startFrom + 1 to High(unitArr) do
    if gMySpectator.Hand.Locks.GetUnitBlocked(unitArr[K]) = ulNotVisible then
      Continue
    else
    begin
      Result := K;
...
begin
  ...
  if gMySpectator.Hand.Locks.GetUnitBlocked(SCHOOL_GAME_ORDER[fLastSchoolUnit]) = ulUnlocked then
  ...
  if gMySpectator.Hand.Locks.GetUnitBlocked(BARRACKS_GAME_ORDER[fLastBarracksUnit]) = ulUnlocked then
  ...
  if gMySpectator.Hand.Locks.GetUnitBlocked(TH_GAME_ORDER[fLastTHUnit]) = ulUnlocked then
...
begin
  ...
  Image_Barracks_Train.Enabled := gMySpectator.Hand.Locks.GetUnitBlocked(BARRACKS_GAME_ORDER[fLastBarracksUnit]) = ulUnlocked;
...
begin
  ...
  Image_TH_Train.Enabled := gMySpectator.Hand.Locks.GetUnitBlocked(TH_GAME_ORDER[fLastTHUnit], True) = ulUnlocked;

  if gMySpectator.Hand.Locks.GetUnitBlocked(TH_GAME_ORDER[fLastTHUnit], True) = ulUnlocked then
...
begin
  ...
  begin
    ...
    Button_School_Train.Enabled := (not school.QueueIsFull)
                                    and (gMySpectator.Hand.Locks.GetUnitBlocked(SCHOOL_GAME_ORDER[fLastSchoolUnit]) = ulUnlocked  );
    ...
    Image_School_Train.Enabled := gMySpectator.Hand.Locks.GetUnitBlocked(SCHOOL_GAME_ORDER[fLastSchoolUnit]) = ulUnlocked;
  ...
  begin
    ...
    Button_Siege_Train.Enabled := (not siege.QueueIsFull)
                                    and (gMySpectator.Hand.Locks.GetUnitBlocked(SIEGE_GAME_ORDER[fLastSiegeUnit]) = ulUnlocked);
    ...
    Image_Siege_Train.Enabled := gMySpectator.Hand.Locks.GetUnitBlocked(SIEGE_GAME_ORDER[fLastSiegeUnit]) = ulUnlocked;
...
begin
  ...
  for I := 0 to 3 do
    ...
    begin
      case WP[I].W of
        wtJewerly: WaresProdCt_Common[1].Caption := 'x' + IntToStr(WP[I].C);
...
begin
  ...
  if J <> -1 then
    ...
    //H.TransferWare[J] := not H.TransferWare[J];
