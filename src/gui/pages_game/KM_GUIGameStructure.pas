...
begin
  ...
  TKMLabel.Create(Panel_Structure, 0, 0, Panel_Structure.Width - 25, 20, gResTexts[2039], fntOutline, taCenter);

  Bar_Progress := TKMPercentBar.Create(Panel_Structure, 10, 15, aParent.Width - 20 - 25, 25, fntMetal);
  Bar_Progress.Position := 0;
  Bar_Progress.Caption := 'Building progress';

  WareCost_Cost := TKMCostsRowMulti.Create(Panel_Structure, 10, 70, aParent.Width - 20 - 25, 21);
  WareCost_Cost.WarePlan.Clear;
  WareCost_Cost.Caption := gResTexts[2037];

  WareCost_Delivered := TKMCostsRowMulti.Create(Panel_Structure, 10, 110, aParent.Width - 20 - 25, 21);
