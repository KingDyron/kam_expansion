...
const
  ProjectileLaunchSounds: array[TKMProjectileType] of TSoundFX = (sfxBowShoot, sfxCrossbowShoot, sfxNone, sfxRockThrow{sfxCatapultShoot}, {sfxCatapultShoot}sfxBalistaShoot, sfxBalistaShoot, sfxCrossbowShoot, sfxBowShoot, sfxBowShoot, sfxNone);
  ProjectileHitSounds:   array[TKMProjectileType] of TSoundFX = (sfxArrowHit, sfxArrowHit, sfxArrowHit, sfxNone, sfxSiegeBuildingSmash, sfxSiegeBuildingSmash, sfxArrowHit, sfxArrowHit, sfxArrowHit, sfxNone);
  ProjectileSpeeds: array[TKMProjectileType] of Single = (0.75, 0.8, 0.6, 1.1, 0.5, 1.6, 1.3, 0.75, 1.5, 0.4);
  ProjectileArcs: array[TKMProjectileType,1..2] of Single = ((1.6, 0.5), (1.4, 0.4), (2.5, 1), (1.2, 0.2), (0.5, 0.2), (0.5, 0.2), (1.4, 0.4), (1.4, 0.4), (0.7, 0.4), (2.5, 1)); //Arc curve and random fraction
  ProjectileJitter: array[TKMProjectileType] of Single = (0.26, 0.29, 0.26, 0.2, 0.29, 0.15, 0.1, 0.1, 0.2, 0.1); //Fixed Jitter added every time
  ProjectileJitterHouse: array[TKMProjectileType] of Single = (0.6, 0.6, 0.6, 0, 0.6, 0.2, 0, 0, 0.3, 0.1); //Fixed Jitter added every time
  // Jitter added according to target's speed (moving target harder to hit) Note: Walking = 0.1, so the added jitter is 0.1*X
  ProjectilePredictJitter: array[TKMProjectileType] of Single = (2, 2, 2, 3, 2, 5, 3, 2, 2, 1);
...
begin
  ...
  begin
    ...
    begin
      ...
      if (U <> nil) and (aOwner <> nil) and (gHands.CheckAlliance(aOwner.Owner, U.Owner) = atAlly) then
...
begin
  ...
  If (aProjType = ptBallistaBolt) and gHands[aOwner.Owner].ArmyDevUnlocked(9) then
    speed := speed * 1.15;
...
const //TowerRock position is a bit different for reasons said below
  OFFSET_X: array [TKMProjectileType] of Single = (0.5, 0.5, 0.5, -0.25, 0.5, 0.5, -0.25, -0.25, 0.5, 0.5); //Recruit stands in entrance, Tower middleline is X-0.75
  OFFSET_Y: array [TKMProjectileType] of Single = (0.2, 0.2, 0.2, -0.2, 0, 0.5, -0.2, -0.25, 0.2, 0.5); //Add towers height
var
  I: Integer;
begin
  I := -1;
...
begin
  ...
  gparticles.AddDustParticle(aTilePos);
end;

procedure TKMProjectiles.HitAlchemist(aOwner: TKMUnit; aTilePos: TKMPointF; aRadius: Single);
begin
  gHands.HitAlchemist(aOwner, aTilePos, aRadius);
  gparticles.AddDustParticle(aTilePos);
end;
    ...
    with fItems[I] do
      ...
      begin
        ...
        begin
          ...
          begin
            case fType of
              ptRedVial: HitAlchemist(fOwner, fTarget, 3);
              ptArrow,
              ptSlingRock,
              ptBallistaBolt,
              ptCatapultRock,
                              ...
                              if H <> nil then
                                ...
                                begin
                                  ...
                                  If gHands[fOwner.Owner].ArmyDevUnlocked(14) then
                                    HDamage := Max(Round(HDamage * 0.7), 0);
                                end;
                                {
                                if fType = ptCatapultRock then
                                  begin
              ...
              ptWallBolt: if (U <> nil) and not U.IsDeadOrDying and U.Visible and not (U is TKMUnitAnimal) then
                          //Can't hit units past max range because that's unintuitive/confusing to player
                          ...
                          begin
                            ...
                            if (Damage >= KaMRandom(101, 'TKMProjectiles.UpdateState:Damage')) or (U.Defence = 0) then
                              U.HitPointsDecrease(UDamage, fOwner);
                          end;
              ptTowerRock:  begin

                              If gHands[fOwner.Owner].ArmyDevUnlocked(22) then
                                HitAllInRadius(fOwner, U, fTarget, 2, 300, KamRandom(3, 'TKMProjectiles.UpdateState: TowerHit') + 2)
                              else
