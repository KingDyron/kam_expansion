...
//    procedure SetFieldNoUpdate(const Loc: TKMPoint; aOwner: TKMHandID; aFieldType: TKMFieldType; aStage: Byte = 0);
    ...
    function TileIsSwamp(X, Y: Word): Boolean; inline;
...
function TKMTerrain.TileIsSwamp(X: Word; Y: Word): Boolean;
begin
  Result := TileHasTerrainKinds(X, Y, [tkSwamp, tkGrassyWater]);
end;
...
begin
  Result := not (Land^[aY, aX].TileLock in [tlRoadWork]) and (
            ...
            or ((RoadDiff(GetRoadType(aX, aY), aRoadType) = 1) and (tpWalkRoad in Land^[aY, aX].Passability) and ((aOwner = Land^[aY, aX].TileOwner) or (Land^[aY, aX].TileOwner = -1)))
