...
begin
  ...
  begin
    ...
    gScriptEvents.ProcUnitHit(fOpponent, fUnit);
    isHit := false;
    fOpponent.SetHitTime;//set hittime no matter if it does damage or not. He is in fight.
    if fOpponent is TKMUnitWarriorSpy then
      TKMUnitWarriorSpy(fOpponent).SetAttackedTime;//Spy can only attack house

    if fUnit.InstantKill then
      fOpponent.HitPointsDecrease(fOpponent.HitPointsMax, fUnit)
    else
    if (fUnit.Attack > 200) then
      fOpponent.HitPointsDecrease(TKMUnitWarrior(fUnit).DamageUnits, fUnit)
    else
    begin
      ...
      if fOpponent.UnitType <> utShieldBearer then
        damage := damage * (GetDirModifier(fUnit.Direction, fOpponent.Direction) + 1); // Direction modifier
      //Defence modifier
      If UNIT_TO_GROUP_TYPE[fUnit.UnitType] <> gtWreckers then
        damage := damage div Math.max(fOpponent.Defence, 1); //Not needed, but animals have 0 defence
