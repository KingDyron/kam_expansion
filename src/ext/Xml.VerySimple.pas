...
type
  TSimpleVariant = record
  private
    // Internal representation is always a string
    fValue: string;
  public
    class operator Implicit(const A: Boolean): TSimpleVariant;
    class operator Implicit(const A: Cardinal): TSimpleVariant;
    class operator Implicit(const A: Integer): TSimpleVariant;
    class operator Implicit(const A: Single): TSimpleVariant;
    class operator Implicit(const A: string): TSimpleVariant;
    class operator Implicit(const A: TDateTime): TSimpleVariant;
    class operator Implicit(const A: TArray<Single>): TSimpleVariant;
    class operator Implicit(const A: TArray<Integer>): TSimpleVariant;
    function AsArrayFloat: TArray<Single>;
    function AsArrayInt: TArray<Integer>;
  ...
  private
    FValue: String;
    function GetValue: TSimpleVariant;
    procedure SetValue(const Value: TSimpleVariant);
  public
    ...
    property Value: TSimpleVariant read GetValue write SetValue;
  end;
  ...
  protected
    ...
    function GetAttr(const AttrName: String): TSimpleVariant; virtual;
    procedure SetAttr(const AttrName: String; const AttrValue: TSimpleVariant); virtual;
  public
    ...
    function SetAttribute(const AttrName: String; const AttrValue: TSimpleVariant): TXmlNode; virtual;
    ...
    property Attributes[const AttrName: String]: TSimpleVariant read GetAttr write SetAttr;
...
begin
  if assigned(FHeader) then
    Result := FHeader.Attributes['encoding'].AsString
  else
    Result := '';
end;
...
begin
  if assigned(FHeader) then
    Result := FHeader.Attributes['standalone'].AsString
  else
    Result := '';
end;
...
begin
  if assigned(FHeader) then
    Result := FHeader.Attributes['version'].AsString
  else
    Result := '';
end;
...
function TXmlNode.GetAttr(const AttrName: String): TSimpleVariant;
var
  Attribute: TXmlAttribute;
begin
...
procedure TXmlNode.SetAttr(const AttrName: String; const AttrValue: TSimpleVariant);
begin
  SetAttribute(AttrName, AttrValue);
end;

function TXmlNode.SetAttribute(const AttrName: String; const AttrValue: TSimpleVariant): TXmlNode;
var
  Attribute: TXmlAttribute;
begin
...
begin
  ...
  for Node in Self do
    if ((NodeTypes = []) or (Node.NodeType in NodeTypes)) and // if no type specified or node type in types
      IsSame(Node.Name, Name) and Node.HasAttribute(AttrName) and IsSame(Node.Attributes[AttrName].AsString, AttrValue) then
    begin
      Result := Node;
      Break;
    end;
...
begin
  ...
  if AttributeType = atSingle then
    Exit;
  Result := Result + '="' + Escape(Value.AsString) + '"';
end;
...
function TXmlAttribute.GetValue: TSimpleVariant;
begin
  Result := FValue;
end;

procedure TXmlAttribute.SetValue(const Value: TSimpleVariant);
begin
...
function TSimpleVariant.AsArrayFloat: TArray<Single>;
var
  sl: TStringList;
  str: string;
  I: Integer;
begin
  ...
  try
    ...
    // Replace delimiters once for all
    ...
    for I := 0 to sl.Count - 1 do
      Result[I] := StrToFloat(sl[I]);
  finally
    sl.Free;
  end;
end;

function TSimpleVariant.AsArrayInt: TArray<Integer>;
var
  sl: TStringList;
  I: Integer;
begin
...
function TSimpleVariant.AsBoolean: Boolean;
begin
  Result := StrToBool(fValue);
end;

function TSimpleVariant.AsBoolean(aDefault: Boolean): Boolean;
begin
  Result := StrToBoolDef(fValue, aDefault);
end;

function TSimpleVariant.AsCardinal: Cardinal;
begin
  Result := StrToInt64(fValue);
end;

function TSimpleVariant.AsCardinal(aDefault: Cardinal): Cardinal;
begin
  Result := StrToInt64Def(fValue, aDefault);
end;

function TSimpleVariant.AsDateTime: TDateTime;
var
  v: Variant;
begin
...
function TSimpleVariant.AsFloat: Single;
var
  str: string;
begin
  str := StringReplace(fValue, '.', FormatSettings.DecimalSeparator, [rfReplaceAll]);
...
function TSimpleVariant.AsFloat(aDefault: Single): Single;
var
  str: string;
begin
  str := StringReplace(fValue, '.', FormatSettings.DecimalSeparator, [rfReplaceAll]);
...
function TSimpleVariant.AsInteger: Integer;
begin
  Result := StrToInt(fValue);
end;

function TSimpleVariant.AsInteger(aDefault: Integer): Integer;
begin
  Result := StrToIntDef(fValue, aDefault);
end;

function TSimpleVariant.AsString: string;
begin
  Result := fValue;
end;

function TSimpleVariant.AsString(aDefault: string): string;
begin
  if fValue <> '' then
    Result := fValue
  else
...
class operator TSimpleVariant.Implicit(const A: Boolean): TSimpleVariant;
const
...
class operator TSimpleVariant.Implicit(const A: Cardinal): TSimpleVariant;
begin
  Result.fValue := '$' + IntToHex(A, 8);
end;

class operator TSimpleVariant.Implicit(const A: Integer): TSimpleVariant;
begin
  Result.fValue := IntToStr(A);
end;

class operator TSimpleVariant.Implicit(const A: Single): TSimpleVariant;
var
  str: string;
  fs: TFormatSettings;
begin
...
class operator TSimpleVariant.Implicit(const A: string): TSimpleVariant;
begin
  Result.fValue := A;
end;

class operator TSimpleVariant.Implicit(const A: TArray<Single>): TSimpleVariant;
var
  I: Integer;
  str: string;
  fs: TFormatSettings;
begin
  ...
  for I := 0 to Length(A) - 1 do
    str := str + IfThen(I > 0, ';') + FormatFloat('0.000', A[I], fs);

  Result.fValue := str;
end;

class operator TSimpleVariant.Implicit(const A: TArray<Integer>): TSimpleVariant;
var
  I: Integer;
  str: string;
begin
  str := '';
  for I := 0 to Length(A) - 1 do
    str := str + IfThen(I > 0, ';') + IntToStr(A[I]);

  Result.fValue := str;
end;

class operator TSimpleVariant.Implicit(const A: TDateTime): TSimpleVariant;
begin
