  ...
  try
    ...
    begin
      ...
      begin
        for I := 0 to gHands.Count - 1 do
          if gHands[I].Enabled then
            gHands[I].Locks.SetUnitBlocked( UNIT_LOCK_FROM_BOOL[aCanTrain], aUnitType);
      end
      else
        gHands[aHand].Locks.SetUnitBlocked(  UNIT_LOCK_FROM_BOOL[aCanTrain], aUnitType)
    end
    else
...
begin
  try
    ...
    and (aType in [Low(UNIT_ID_TO_TYPE) .. High(UNIT_ID_TO_TYPE)]) then
      gHands[aHand].Locks.SetUnitBlocked(UNIT_LOCK_FROM_BOOL[aBlock], UNIT_ID_TO_TYPE[aType])
    else
